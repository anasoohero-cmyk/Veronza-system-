...
async function registerVeronzaPasskey(){try{if(!sb)throw Error('سجّل الدخول أولاً');if(!window.PublicKeyCredential||!navigator.credentials)throw Error('هذا الجهاز/المتصفح لا يدعم Passkey');let {data,error}=await sb.auth.registerPasskey({friendlyName:'Veronza Face ID'});if(error)throw error;localStorage.setItem('veronza_passkey_registered','1');let card=$('vzRegisterPasskeyCard');if(card){card.innerHTML='<div style="display:flex;align-items:center;justify-content:space-between;gap:14px;flex-wrap:wrap;"><div><div style="font-size:20px;font-weight:800;margin-bottom:5px;">✅ Face ID مفعّل</div><div style="font-size:14px;color:#766d72;line-height:1.7;">تم حفظ Passkey لهذا الحساب. من شاشة الدخول استخدم «دخول سريع بـ Face ID».</div></div><button class="primary" type="button" onclick="passkeyLogin(false)" style="min-width:210px;">🔐 دخول سريع بـ Face ID</button></div>';}toast('تم حفظ Face ID بنجاح');return {ok:true,data}}catch(e){console.error(e);toast('تعذر تفعيل Face ID: '+(e.message||'خطأ'));return {ok:false,error:e}}}
function ensurePasskeyButton(){
  const card=$('vzRegisterPasskeyCard');
  if(!card)return;
  card.style.display='block';
  if(localStorage.getItem('veronza_passkey_registered')==='1'){
    card.innerHTML='<div style="display:flex;align-items:center;justify-content:space-between;gap:14px;flex-wrap:wrap;"><div><div style="font-size:20px;font-weight:800;margin-bottom:5px;">✅ Face ID مفعّل</div><div style="font-size:14px;color:#766d72;line-height:1.7;">Passkey محفوظ لهذا الحساب. استخدمه للدخول السريع.</div></div><button class="primary" type="button" onclick="passkeyLogin(false)" style="min-width:210px;">🔐 دخول سريع بـ Face ID</button></div>';
  }
}
