...
function cfg(){try{let c=JSON.parse(localStorage.getItem('veronza_supabase')||'null');if(c&&c.url){let u=normalizeSupabaseUrl(c.url);if(u!==c.url){c.url=u;localStorage.setItem('veronza_supabase',JSON.stringify(c))}}return c}catch{return null}}
...
function saveConfig(){let url=normalizeSupabaseUrl($('cfgUrl').value);let key=$('cfgKey').value.trim();if(!url||!key)return toast('أدخل الرابط والمفتاح العام');if(!/^https:\/\/[a-z0-9-]+\.supabase\.co$/i.test(url))return toast('رابط Supabase غير صحيح');localStorage.setItem('veronza_supabase',JSON.stringify({url,key}));location.reload()}
...
async function boot(){let c=cfg();if(!c){$('setup').style.display='flex';return}try{sb=window.supabase.createClient(c.url,c.key,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});sb.auth.onAuthStateChange((event,session)=>{if(event==='SIGNED_OUT'){stopRealtime();location.reload()}else if(event==='SIGNED_IN'&&session){$('login').style.display='none';startRealtime();scheduleLoadAll()}});let {data:{session},error:sessionError}=await sb.auth.getSession();if(sessionError)throw sessionError;if(!session){$('login').style.display='flex';return}document.querySelectorAll('nav button').forEach(b=>b.onclick=()=>showView(b.dataset.v));let u=document.querySelector('.user');u.innerHTML='مرحباً بك <b>'+((session.user.email||'').split('@')[0])+' ⌄</b>';u.onclick=()=>sb.auth.signOut();await loadAll();startRealtime()}catch(e){console.error(e);$('setup').style.display='flex';toast('تعذر الاتصال بـ Supabase: '+(e.message||'خطأ غير معروف'))}}
async function login(){try{let c=cfg();if(!c)throw Error('إعداد Supabase غير موجود');sb=window.supabase.createClient(c.url,c.key,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});let {error}=await sb.auth.signInWithPassword({email:$('loginEmail').value.trim(),password:$('loginPassword').value});$('loginMsg').textContent=error?error.message:'تم الدخول';if(!error)location.reload()}catch(e){$('loginMsg').textContent=e.message||'تعذر تسجيل الدخول'}}
async function signup(){try{let c=cfg();if(!c)throw Error('إعداد Supabase غير موجود');sb=window.supabase.createClient(c.url,c.key,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});let {error}=await sb.auth.signUp({email:$('loginEmail').value.trim(),password:$('loginPassword').value});$('loginMsg').textContent=error?error.message:'تم إنشاء الحساب. تحقق من البريد إذا طلب منك ذلك.'}catch(e){$('loginMsg').textContent=e.message||'تعذر إنشاء الحساب'}}
